...
insert into car_rental_companies (name)
values ("Gertz Car Rental");

insert into Locations (name, hours_of_operation, additional_information, car_rental_id)
...
Car Pick-Up and Drop-Off area where car rental shuttles will be waiting to pick up and drop off customers.', 1);
...
insert into Cars (license_plate, model_year, color, model_id, location_id)
values('ABC1234', 2018, 'Obsydian Black', 1, 1);
...
insert into Drivers_Licenses (customer_id, number, valid_from, expiration) 
values (2, 5487921, '2019-06-13', '2020-06-13');

insert into Credit_Cards (number, valid_from, expiration, customer_id, type_id)
...
insert into Payments (type_id, rental_id, date)
values (1, 1, "2020-04-04");
...
left join People p on be.id = p.id
left join Employees emp on emp.id = p.id
left join Locations loc on loc.id = be.id
left join Car_Rental_Companies cr on loc.car_rental_id = cr.id
left join Addresses addr on loc.id = addr.business_entity_id
left join Regions reg on addr.region_id = reg.id
left join Countries countr on reg.country_id = countr.id
...
left join Rentals r on r.employee_id = emp.id
...
left join Customers cust on p.id = cust.id
left join Drivers_Licenses dl on dl.customer_id = cust.id
left join Credit_Cards cc on cc.customer_id = cust.id
...
on r.pickup_location_id = loc.id
